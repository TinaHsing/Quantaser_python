...
#connectbutton = Button(master, text = "Connect", command = spcm.BuildConnection(ser, errmsg))
errmsg.grid(column =0, row=0) 
#connectbutton.grid(column =1, row =0)
nb.grid(column =0, row =1, columnspan = 2)
...
cp =0
idle = list([0]) # init state machine
...
def LoadData():
	global ser
	global askdc, askti, sendreset, idle
	global ch0coarse, ch0fine, ch1coarse, ch1fine
	global runindex1
	runindex1 = 0
	exp1_thread.join()
	spcm.LoadData2(ser, askdc, askti, sendreset, idle)
	#spcm.LoadData(ch0coarse, ch0fine, ch1coarse, ch1fine, ser, askdc, askti, sendreset, idle)
	#print ch0coarse
def Exp1LoadButton():
	global exp1_thread
	global exp1_thread2
...
def TimeAnalysis2():
	...
	if timegetstr1 =="":
		...
		if chselect.get() == "CH0":
			countarray = spcm.CoarseHistoAnalysis2(0, int_timeset)
		else:
			countarray = spcm.CoarseHistoAnalysis2(1, int_timeset)
...
Anabutton1 = Button(exp1, text ="Analysis", command = TimeAnalysis2)
